...
export const appConfig = () => ({
  ...
  stream_config: {
    exchange_stream_name: `${process.env.EXCHANGE_ID}-client`,
    transaction_stream_name: `${process.env.CHAIN_ID}-transactions`,
    logs_stream_name: `${process.env.CHAIN_ID}-logs`,
    block_stream_name: `${process.env.CHAIN_ID}-blocks`,
    queue_url: `${process.env.CHAIN_ID}-blocks`,
  },
  config: {
    full_node:
      process.env.NODE_RPC || `https://${process.env.CHAIN_ID}.nodes.prod.euler.tools/rpc` || '',
    full_wss:
